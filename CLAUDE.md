...
5. **Överlämning (åt båda hållen):**
   ...
   - 🧭 **Rutan `VAR PROJEKTET STÅR` överst i `docs/HANDOFF.md` är STÅENDE och skrivs om PÅ PLATS.** Den är inte en sessionssektion och ska aldrig staplas eller dupliceras. Uppdatera den **innan** du skriver din egen sektion: fas, var i fasen, nästa jobb, vad som blockerar. **Kravet står här och inte i skillfilen** — den kopian bär en not om att innehålla exakt en avvikelse från Matt Pococks original, så att den går att synka om. Skälet rutan finns: loggen svarar på *"vad hände sist"* men aldrig på *"var är vi"*, och Adam bad om det 2026-08-27 efter tre sessioner där han inte kunde se det ur dokumenten.
